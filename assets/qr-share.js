 ...
 *
 * LOCAL INTEGRATION NOTES (this portfolio):
 *   - QR_LIB_URL points at a vendored copy (assets/vendor/) instead of the CDN,
 *     so the widget makes ZERO external/CDN calls at runtime.
 *   - The Google Fonts <link> injection was removed: the host page already
 *     self-hosts "Space Grotesk" and "IBM Plex Mono" via assets/fonts/fonts.css,
 *     so the HUD reuses those (no fonts.googleapis.com request).
 * ==========================================================================*/
(function () {
  'use strict';
  ...
  function injectStyles(accent) {
    ...
    // No webfont fetch: the host page self-hosts "Space Grotesk" and
    // "IBM Plex Mono" (assets/fonts/fonts.css); the HUD reuses them and falls
    // back to system fonts where unavailable.
    var css = [
